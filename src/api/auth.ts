...
import { getTokenToLocalStorage } from "../lib/token";
...
export const postSignup = async ({ email, password }: formValues) => {
  return await customAxios.post(`/accounts:signUp?key=${process.env.REACT_APP_API_KEY}`, {
    email: email,
    password: password,
...
export const postVerifyToken = async () => {
  return await axios.post(
    `https://securetoken.googleapis.com/v1/token?key=${process.env.REACT_APP_API_KEY}`,
    {
      ...
      refresh_token: getTokenToLocalStorage(),
    }
  );
};
